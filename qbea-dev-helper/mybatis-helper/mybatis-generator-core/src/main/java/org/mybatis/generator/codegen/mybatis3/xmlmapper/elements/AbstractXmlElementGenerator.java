...
public abstract class AbstractXmlElementGenerator extends AbstractGenerator {
     ...
     */
    protected XmlElement getMysqlSelectKey(IntrospectedColumn introspectedColumn,
            GeneratedKey generatedKey) {
        String identityColumnType = introspectedColumn
                .getFullyQualifiedJavaType().getFullyQualifiedName();
        /** qbea @todo 当xml中table配置了selectKey SqlStatement 会被固定，目前支持Mysql/Postgresql **/
        XmlElement answer = new XmlElement("selectKey"); //$NON-NLS-1$
        answer.addAttribute(new Attribute("resultType", identityColumnType)); //$NON-NLS-1$
        answer.addAttribute(new Attribute("databaseId", "mysql"));
        ...
        answer.addElement(new TextElement("SELECT LAST_INSERT_ID()"));

        return answer;
    }

    protected XmlElement getPostgreSqlSelectKey(IntrospectedColumn introspectedColumn,
                                           GeneratedKey generatedKey) {
        String identityColumnType = introspectedColumn
                .getFullyQualifiedJavaType().getFullyQualifiedName();
        /** qbea @todo 当xml中table配置了selectKey SqlStatement 会被固定，目前支持Mysql/Postgresql **/
        XmlElement answer = new XmlElement("selectKey"); //$NON-NLS-1$
        answer.addAttribute(new Attribute("resultType", identityColumnType)); //$NON-NLS-1$
        answer.addAttribute(new Attribute("databaseId", "postgresql"));
        answer.addAttribute(new Attribute(
                "keyProperty", introspectedColumn.getJavaProperty())); //$NON-NLS-1$
        answer.addAttribute(new Attribute("order", //$NON-NLS-1$
                generatedKey.getMyBatis3Order()));
        ...
        answer.addElement(
                new TextElement(
                        "SELECT currval('"
                                +introspectedColumn.getIntrospectedTable().getFullyQualifiedTableNameAtRuntime()
                                +"_seq')"));
